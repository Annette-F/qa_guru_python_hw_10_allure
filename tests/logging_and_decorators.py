...
def step(fn):
    ...
    def fn_with_logging(*args, **kwargs):
        ...
        args_kwargs_to_log_as_strings = [
            *map(str, args_to_log),
            *[f'{key}={value}' for key, value in kwargs.items()]
        ]
        args_and_kwargs_string = (
            (': ' + ', '.join(map(str, args_kwargs_to_log_as_strings)))
            if args_kwargs_to_log_as_strings
            else ''
        )
        print(
...
(sign_up_form
 .fill_name('Anna', surname='Fedorova')
 .fill_email('example@mail.com')
